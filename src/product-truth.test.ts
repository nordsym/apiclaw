...
assert.equal(packageMetadata.version, "2.9.10");
assert.equal(packageLock.version, "2.9.10");
assert.equal(packageLock.packages[""].version, "2.9.10");
...
for (const healthFile of ["api/health.ts", "landing/pages/api/health.ts"]) {
  ...
  assert.match(health, /service: 'apiclaw-gateway'/);
    assert.match(health, /version: '2\.9\.10'/);
}
...
assert.equal(mcpbManifest.version, "2.9.10");
assert.doesNotMatch(
