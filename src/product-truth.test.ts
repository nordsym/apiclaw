...
assert.equal(packageMetadata.version, "2.8.9");
assert.equal(packageLock.version, "2.8.9");
assert.equal(packageLock.packages[""].version, "2.8.9");
...
for (const healthFile of ["api/health.ts", "landing/pages/api/health.ts"]) {
  ...
  assert.match(health, /version: '2\.8\.9'/);
}
...
assert.equal(mcpbManifest.version, "2.8.9");
assert.doesNotMatch(
