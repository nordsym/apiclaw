...
assert.equal(packageMetadata.version, "2.9.11");
assert.equal(packageLock.version, "2.9.11");
assert.equal(packageLock.packages[""].version, "2.9.11");
...
for (const healthFile of ["api/health.ts", "landing/pages/api/health.ts"]) {
  ...
  assert.match(health, /service: 'apiclaw-gateway'/);
    assert.match(health, /version: '2\.9\.11'/);
}
...
assert.equal(mcpbManifest.version, "2.9.11");
assert.doesNotMatch(
