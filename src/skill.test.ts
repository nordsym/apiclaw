...
assert.match(skill, /Show the human the login URL|Show the human that URL|Show the human the live|Show the human that first-line URL/);
assert.match(skill, /auth\/cli\?authId=/);
assert.match(skill, /Do not only print/);
assert.match(skill, /own line|on its own line|first line|first-line/);
assert.match(skill, /Do not summarize/);
assert.match(skill, /machine the human cannot see/);
assert.match(skill, /do not run `auth login` there/);
assert.match(skill, /one action/);
assert.match(skill, /stay in this chat|continue in this (?:chat|session)/);
assert.match(skill, /Do not tell them to open Terminal\.app|Do not tell the human to open Terminal\.app/);
assert.match(skill, /Workspace is optional/);
assert.doesNotMatch(skill, /Authorizes the terminal/);
...
assert.match(agents, /continue in this same chat/);
assert.match(agents, /Do not send the human to a terminal or Terminal\.app/);
assert.match(agents, /Workspace is optional/);
