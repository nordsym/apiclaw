...
const N8N_BOOKING = "https://nordsym.app.n8n.cloud/webhook/hivr-booking";
...
function CalendarPicker({
  ...
  const monthName = new Date(state.calYear, state.calMonth, 1).toLocaleDateString("en-US", {
    month: "long",
    year: "numeric",
  });
    ...
    cells.push(
      <button
        ...
        className={`w-full text-center text-xs py-1.5 rounded-lg transition-colors
          ...
          ${isPast ? "text-neutral-300 cursor-not-allowed" : !isSel ? "hover:border hover:border-red-300 text-neutral-700" : ""}
        `}
      >
        {day}
      </button>
...
function TimeSlots({
  ...
  if (!state.selectedDate) {
    return (
      <div className="border border-neutral-200 rounded-xl p-4 flex items-center justify-center h-full">
        <p className="text-xs text-neutral-400 text-center">Select a date to see available times</p>
      </div>
    );
  }
  ...
  return (
    <div className="border border-neutral-200 rounded-xl p-4">
      ...
      <p className="text-xs text-neutral-400 mt-3">{tzShort} time · 3 slots taken</p>
    </div>
  );
}
...
function WelcomeFlow() {
  ...
  const [state, setState] = useState<State>(() => {
    ...
    return {
      step: 1,
      name: searchParams?.get("signerName") || "Pratham Shah",
      email: "pratham@apilayer.com",
      title: searchParams?.get("signerTitle") || "",
      company: "APILayer",
      primaryGoal: "",
      priorityApis: "",
      channels: "",
  ...
  const handleBook = async () => {
    ...
    try {
      const res = await fetch(N8N_BOOKING, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          customerId: "apilayer",
          customerName: "APILayer",
          ...
          agenda: state.agenda,
          partnershipGoal: state.primaryGoal,
          priorityApis: state.priorityApis,
          teamEmails: ["gustav@nordsym.com", "molle@cqtinvest.com"],
          source: "apiclaw-sow-onboarding",
        }),
      });
      ...
      setBookingError("Something went wrong. Please email gustav@nordsym.com directly.");
    }
  };
  ...
  const renderStep1 = () => (
    <div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {[
          { label: "Full name", key: "name", value: state.name },
          { label: "Work email", key: "email", value: state.email },
          { label: "Title", key: "title", value: state.title },
          { label: "Company", key: "company", value: state.company },
        ].map(({ label, key, value }) => (
          <div key={key}>
            ...
            <input
              ...
              value={value}
              onChange={(e) => update({ [key]: e.target.value } as Partial<State>)}
      ...
      <div className="flex justify-end mt-4">
        <button
          onClick={() => {
            if (!state.name || !state.email) return alert("Name and email required.");
            next();
          }}
    ...
    return (
      <div>
        ...
        <div className="flex justify-between mt-4">
          ...
          <button
            onClick={() => {
              if (!agenda.trim()) return alert("Please keep an agenda.");
              update({ agenda });
              next();
            }}
            className="bg-gradient-to-r from-red-600 to-red-700 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity"
      ...
      return (
        <div className="text-center py-8">
          ...
          <p className="text-neutral-600 text-sm mb-1">
            Check <strong>{state.email}</strong> for your Google Meet invite and confirmation.
          </p>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="p-8">
            ...
            <p className="text-neutral-500 text-sm mb-6">
              Let&apos;s set up your partnership and book a kickoff.
            </p>

            <div className="h-1 w-16 bg-gradient-to-r from-red-500 to-red-600 rounded-full mb-6" />
