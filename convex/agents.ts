...
/** True when `name` is exactly one of the "Adjective Noun" combos generateAgentName() could produce. */
...
export const renameAgent = mutation({
  ...
  handler: async (ctx, { token, agentId, name }) => {
    ...
    await ctx.db.patch(agentId, { name: trimmedName, nameSetByUser: true });
  ...
  handler: async (ctx, { fingerprint, mcpClient, platform, sessionToken }) => {
    ...
    if (sessionToken) {
      ...
      if (session) {
        ...
        // `name` still gets a generated placeholder for backward compat
        // (older clients/exports may read it directly) but nameSetByUser
        // stays unset, so display resolution never treats it as a real
        // user-chosen name; see agentDisplay.ts.
  ...
  handler: async (ctx, { token }) => {
    ...
    return agents.map((a) => ({
      ...
      displayName: resolveAgentDisplayName({
        userSetName: a.nameSetByUser ? a.name : null,
        mcpClient: a.mcpClient,
        fallbackName: a.name,
      }),
...
/**
 * One-time backfill (real names, 2026-08-24): nameSetByUser didn't exist
 * before this change, so every pre-existing agent row reads as "not user
 * set" by default, which would silently override a name someone already
 * typed in through renameAgent (e.g. "Symbot") with the prettified
 * mcpClient. Any stored name that does NOT match the "Adjective Noun"
 * shape generateAgentName() produces is treated as a real rename and
 * flagged nameSetByUser: true. Idempotent, safe to re-run.
 */
export const backfillNameSetByUser = internalMutation({
  args: {},
  handler: async (ctx) => {
    ...
    for (const a of agents) {
      if (a.nameSetByUser) { skipped++; continue; }
      if (a.name && !looksAutoGenerated(a.name)) {
        await ctx.db.patch(a._id, { nameSetByUser: true });
        flagged++;
      }
    }
    return { total: agents.length, flagged, skipped };
  },
});
